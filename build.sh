...
# --- Corrections automatiques des rejets ---
...
echo "=== Recherche du binaire ksud réellement produit ==="
find "$GITHUB_WORKSPACE/ksud-src/target" -type f -executable 2>/dev/null | grep -v "\.d$\|\.rlib$\|build/" || true
echo "=== Contenu du dossier release (target aarch64-linux-android) ==="
ls -la "$GITHUB_WORKSPACE/ksud-src/target/aarch64-linux-android/release/" 2>/dev/null || echo "Ce dossier n'existe pas"
echo "=== Contenu du dossier release (target générique, sans triple) ==="
ls -la "$GITHUB_WORKSPACE/ksud-src/target/release/" 2>/dev/null || echo "target/release/ n'existe pas non plus"

# Essai automatique : chercher un binaire nommé ksud n'importe où sous target/
KSUD_BINARY=$(find "$GITHUB_WORKSPACE/ksud-src/target" -type f -name "ksud" -executable 2>/dev/null | head -1)

if [ -z "$KSUD_BINARY" ]; then
    echo "❌ ksud introuvable après recherche automatique — voir le diagnostic ci-dessus pour le vrai nom/emplacement"
    exit 1
fi
