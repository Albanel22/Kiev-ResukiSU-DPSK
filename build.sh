...
# ==================== 6c. VÉRIFICATION KSUD_PATH ET EMBED ====================
echo ""
echo "=== Vérification KSUD_PATH et extraction ksud ==="
cd "$GITHUB_WORKSPACE/kernel_sources"

echo "--- Où le kernel cherche ksud ? ---"
grep -rn "KSUD_PATH\|ksud_path" drivers/kernelsu/ 2>/dev/null | head -10 || echo "(aucune référence KSUD_PATH)"

echo ""
echo "--- Est-ce que ksud est embarqué dans le kernel ? ---"
grep -rn "EMBED_KSUD\|embed_ksud\|ksud_embed\|incbin.*ksud" drivers/kernelsu/ 2>/dev/null | head -10 || echo "(aucun mécanisme EMBED trouvé)"

echo ""
echo "--- Fichiers liés à ksud dans drivers/kernelsu/ ---"
find drivers/kernelsu -name "*ksud*" -o -name "*.incbin*" 2>/dev/null | head -20 || echo "(aucun fichier ksud dédié)"

echo ""
echo "--- Options Kconfig liées à ksud ---"
grep -rn "KSUD\|EMBED" drivers/kernelsu/Kconfig 2>/dev/null | head -20 || echo "(aucune option Kconfig)"

echo ""
echo "--- Chemins '/data/adb' dans le binaire kernel ---"
strings out/arch/arm64/boot/Image 2>/dev/null | grep -E "^/data|/data/adb" | head -20 || echo "(aucun chemin data/adb)"
