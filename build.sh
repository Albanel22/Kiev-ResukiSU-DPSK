...
# ==================== DIAGNOSTIC : localiser le vrai binaire produit (recherche elargie) ====================
echo "=== Recherche du binaire ksud dans tout le repo cloné ==="
find "$GITHUB_WORKSPACE/ksud-src" -type f -name "ksud" 2>/dev/null
echo "=== Recherche de n'importe quel dossier target/ ==="
find "$GITHUB_WORKSPACE/ksud-src" -maxdepth 3 -type d -name "target" 2>/dev/null
echo "=== Contenu de la racine du repo ksud-src ==="
ls -la "$GITHUB_WORKSPACE/ksud-src/"

# Essai automatique : chercher un binaire nommé ksud n'importe où dans le repo cloné
