...
# 3b. FIX VARIABLE 'vma' NON UTILISÉE (-Werror) DANS task_mmu.c
# Corrige "error: unused variable 'vma' [-Werror,-Wunused-variable]" qui survient
# même quand le patch SuSFS s'applique sans rejet (donc en plus du bloc .rej ci-dessus).
if [ -f "fs/proc/task_mmu.c" ]; then
    sed -i 's/struct vm_area_struct \*vma;/struct vm_area_struct *vma __maybe_unused;/g' fs/proc/task_mmu.c
    echo "✅ Fix vma __maybe_unused appliqué à task_mmu.c"
fi
